    ...
    $.fn.improveDropDown = function (value) {
            ...
            if (arguments.length === 0 || typeof value === 'object') {
                    ...
                    if (arguments.length === 1) {
                        if ('iconPath' in value) {
                            iconPath = value.iconPath.toString();
                        }
                        if ('noMatchesText' in value) {
                            noMatchesText = value.noMatchesText.toString();
                        }
                        if ('noItemsText' in value) {
                            noItemsText = value.noItemsText.toString();
                        }
						if ('disableRemoteLogos' in value) {
                            disableRemoteLogos = value.disableRemoteLogos;
                        }
                    }
...
function getTextElement(sourceElement, imgElement) {
    ...
    newTextElement.keyup(function (e) {
        ...
        switch (e.which) {
            case KEY_ESCAPE:
                closeListUndoTyping(thisTextElement);
                break;
            case KEY_ENTER:
            ...
            case KEY_RIGHTARROW:
                break;
            default:
                delayCall('updatefilter_' + this.id, function () { updateListFilter(thisTextElement); }, KEY_PRESS_FILTER_DELAY_MS);
                break;
        }
    });
...
function getImageElement(sourceElement,iconPath) {
    ...
    newImgElement.mouseenter(function () {
        suspendTextBoxExitHandler = true;
    });
    newImgElement.mouseleave(function () {
        suspendTextBoxExitHandler = false;
    });
...
function populateListItem(newListControl, optionItem) {
    ...
    if ((optionItem.attr("disabled") || '') == '') {
        newListItem.mouseover(function () {
            jq(this).parent()
                    .find('.idd_listItem_Hover')
                    .removeClass('idd_listItem_Hover');
            jq(this).addClass('idd_listItem_Hover');
        });
        newListItem.mouseout(function () {
            jq(this).removeClass('idd_listItem_Hover');
        });
...
function getItemValue(item) {
    return item.attr('savedValue');
}

function setItemValue(item, value) {
    item.attr('savedValue', value);
}
...
function selectFirstMatch(textControl,closeList) {
    ...
    if ( getIsDirty(textControl) || updateKey in delayedCalls ) {
        ...
        if (bestMatchItem == null) {
            ...
            selectItem(bestMatchItem, true, closeList, false);
        }
    }
...
function navItem(textControl, navDirection) {
    ...
    if ((selectedItem != null) && (selectedItem.length>0)) {
        ...
        if (isItemSelected) {
            listControl.find('.idd_listItem_Hover').removeClass('idd_listItem_Hover');
        }
...
function getTextControlFromOtherControl(Control) {
    return swapControlSuffix(Control, idd_text_suffix);
}

function getListControlFromOtherControl(Control) {
    return swapControlSuffix(Control, idd_list_suffix);
}

function getImageControlFromOtherControl(Control) {
    return swapControlSuffix(Control, idd_icon_suffix); }
function getSelectControlFromOtherControl(Control) { return swapControlSuffix(Control, '');
}

function isOptGroup(control) {
    return control.get(0).tagName.toUpperCase() == 'OPTGROUP';
}
...
function stringContainsCaseInsensitive(SearchIn, SearchFor) {
...
function delayCall(key, func, delayMs) {
    ...
    delayedCalls[key] = setTimeout(function () {
        processDelayedCall(key, func);
    } ,delayMs);
}

function processDelayedCall(key, func) {
     if (key in delayedCalls) {
         delete delayedCalls[key];
     }
     func();
 }
